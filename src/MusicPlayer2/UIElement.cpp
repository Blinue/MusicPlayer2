...
int UiElement::Element::GetWidth(CRect parent_rect) const
{
	...
	w = std::min(GetMaxWidth(parent_rect), w);
	if (min_width.IsValid())
		w = std::max(min_width.GetValue(parent_rect), w);
	return w;
}
...
int UiElement::Element::GetHeight(CRect parent_rect) const
{
	...
	if (max_height.IsValid())
		h = std::min(max_height.GetValue(parent_rect), h);
	if (min_height.IsValid())
		h = std::max(min_height.GetValue(parent_rect), h);
	return h;
}
			...
			else
			{
				...
				for (size_t i{}; i < childLst.size(); ++i)  // 计算比例系数和
				{
					if (size_list[i] == INT_MIN)
						proportion += std::max(childLst[i]->proportion, 1);  // 均未设置时按1处理
				}
				// 逐个检查是否符合最值
				bool ok{ true };
				for (size_t i{}; i < childLst.size(); ++i)
				{
					if (size_list[i] == INT_MIN)
					{
						...
						int size{ (GetRect().Width() - total_size) * std::max(child->proportion, 1) / proportion };
		...
		while (!all_ok)
		{
			...
			else
			{
				...
				for (size_t i{}; i < childLst.size(); ++i)
				{
					...
					{
						auto& child{ childLst[i] };
						int size{ (GetRect().Height() - total_size) * std::max(child->proportion, 1) / proportion };
...
int UiElement::Button::GetMaxWidth(CRect parent_rect) const
{
	...
	if (show_text && !width.IsValid())
	{
		...
		return std::min(width_text, width_max);
	}
	else
	{
