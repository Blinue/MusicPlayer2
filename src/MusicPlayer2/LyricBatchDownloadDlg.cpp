...
BOOL CLyricBatchDownloadDlg::OnInitDialog()
{
	...
	for (size_t i{}; i < m_playlist.size(); i++)
	{
		CString tmp;
		tmp.Format(_T("%d"), i + 1);
		m_song_list_ctrl.InsertItem((int)i, tmp);
		m_song_list_ctrl.SetItemText((int)i, 1, m_playlist[i].GetTitle().c_str());
		m_song_list_ctrl.SetItemText((int)i, 2, m_playlist[i].GetArtist().c_str());
		m_song_list_ctrl.SetItemText((int)i, 3, m_playlist[i].GetFileName().c_str());
	}
...
void CLyricBatchDownloadDlg::OnBnClickedStartDownload()
{
	...
	for (size_t i{}; i < m_playlist.size(); i++)
	{
		m_song_list_ctrl.SetItemText((int)i, 4, _T(""));
	}
...
UINT CLyricBatchDownloadDlg::ThreadFunc(LPVOID lpParam)
{
	...
	for (size_t i{}; i < pInfo->playlist->size(); i++)
	{
		if (theApp.m_batch_download_dialog_exit)
			return 0;
		int percent = (int)i * 100 / (int)pInfo->playlist->size();
		...
		pInfo->list_ctrl->SetItemText((int)i, 4, theApp.m_str_table.LoadText(L"TXT_LYRIC_BDL_STATUS_DOWNLOADING").c_str());
		pInfo->list_ctrl->EnsureVisible((int)i, FALSE);
		...
		{
			pInfo->list_ctrl->SetItemText((int)i, 4, theApp.m_str_table.LoadText(L"TXT_LYRIC_BDL_STATUS_SKIPPED").c_str());
			continue;
		}
		...
		if (rtn != 0)
		{
			pInfo->list_ctrl->SetItemText((int)i, 4, theApp.m_str_table.LoadText(L"TXT_LYRIC_BDL_STATUS_NETWORK_FAILED").c_str());
			continue;
		}
		...
		if (down_list.empty())
		{
			pInfo->list_ctrl->SetItemText((int)i, 4, theApp.m_str_table.LoadText(L"TXT_LYRIC_BDL_STATUS_CANNOT_FIND_THIS_SONG").c_str());
		...
		if (best_matched < 0)
		{
			song_info_ori.SetNoOnlineLyric(true);
			CSongDataManager::GetInstance().AddItem(song_info_ori);
			pInfo->list_ctrl->SetItemText((int)i, 4, theApp.m_str_table.LoadText(L"TXT_LYRIC_BDL_STATUS_NO_MATCHED_LYRIC").c_str());
			continue;
		}
		...
		if (lyric_str.empty())
		{
			pInfo->list_ctrl->SetItemText((int)i, 4, theApp.m_str_table.LoadText(L"TXT_LYRIC_BDL_STATUS_DOWNLOAD_FAILED").c_str());
			continue;
		}
		...
		{
			pInfo->list_ctrl->SetItemText((int)i, 4, theApp.m_str_table.LoadText(L"TXT_LYRIC_BDL_STATUS_SONG_NO_LYRIC").c_str());
			continue;
		}
		...
		if (CLyricBatchDownloadDlg::SaveLyric(lyric_path.c_str(), lyric_str, pInfo->save_code, &char_cannot_convert))
		{
			if (char_cannot_convert)
				pInfo->list_ctrl->SetItemText((int)i, 4, theApp.m_str_table.LoadText(L"TXT_LYRIC_BDL_STATUS_ENCODE_WARNING").c_str());    //char_cannot_convert为true，则说明有无法转换的Unicode字符
			else
				pInfo->list_ctrl->SetItemText((int)i, 4, theApp.m_str_table.LoadText(L"TXT_LYRIC_BDL_STATUS_SUCCEEDED").c_str());
		}
		else
		{
			pInfo->list_ctrl->SetItemText((int)i, 4, theApp.m_str_table.LoadText(L"TXT_LYRIC_BDL_STATUS_SUCCEEDED").c_str());
