...
void CPlayListCtrl::QuickSearch(const wstring & key_word)
{
	...
	for (size_t i{}; i < m_all_song_info.size(); i++)
	{
		if (theApp.m_chinese_pingyin_res.IsStringMatchWithPingyin(key_word, m_all_song_info[i].GetFileName())
			...
			m_search_result.push_back((int)i);
	}
}
	...
	*pResult = CDRF_DODEFAULT;
	...
	case CDDS_ITEMPREPAINT:			//如果为画ITEM之前就要进行颜色的改变
		if (IsWindowEnabled())
		{
			...
			{
				if (GetItemState((int)nmcd.dwItemSpec, LVIS_SELECTED) == LVIS_SELECTED)	//不允许选中行
					SetItemState((int)nmcd.dwItemSpec, 0, LVIS_SELECTED);
			...
			else
			{
				...
				else		//如果播放列表处于搜索状态，则高亮项目应该为搜索结果的索引
				{
					...
					if (iter == m_search_result.end())
						...
						highlight_item = int(iter - m_search_result.begin());
				}
				//当选中行又是高亮行时设置颜色
				if (GetItemState((int)nmcd.dwItemSpec, LVIS_SELECTED) == LVIS_SELECTED && nmcd.dwItemSpec == highlight_item)
				{
